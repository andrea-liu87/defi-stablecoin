...
contract Handler is Test {
    ...
    function depositCollateral(uint256 collateralSeed, uint256 amountCollateral) public {
     ...
     function minstDsc(uint256 collateralSeed, uint256 mintAmount) public {
        address collateralToken = _getCollateralToken(collateralSeed);
        uint256 collateralBalance = dsce.getCollateralBalanceOfUser(msg.sender, collateralToken);
        mintAmount = bound(mintAmount, 0, collateralBalance);
        if(mintAmount == 0){
            return;
        }
        vm.prank(msg.sender);
